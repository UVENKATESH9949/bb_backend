...
public class MockSessionServiceImpl implements MockSessionService {
 ...
 // ── JSON helper — add this to MockSessionServiceImpl ──────────────
    ...
    public MockSessionResponse startMock(Long userId, MockSessionRequest request) {
        ...
        Optional<MockSession> activeMock = getLatestActiveMock(userId, true);
        ...
        if (request.getTotalQuestions() != null && request.getTotalQuestions() > 0
                && selectedQuestions.size() > request.getTotalQuestions()) {
            selectedQuestions = new ArrayList<>(
                selectedQuestions.subList(0, request.getTotalQuestions()));
        }
        ...
        initializeAnswerSheet(session, selectedQuestions);
    ...
    public MockSessionResponse getActiveMock(Long userId) {
        MockSession session = getLatestActiveMock(userId, true)
            .orElseThrow(() -> new ResourceNotFoundException(
    ...
    private Optional<MockSession> getLatestActiveMock(Long userId, boolean autoAbandonStale) {
        List<MockSession> activeMocks = mockSessionRepository
            .findByUserIdAndStatusOrderByCreatedAtDesc(userId, MockStatus.IN_PROGRESS);

        if (activeMocks.isEmpty()) {
            return Optional.empty();
        }

        MockSession latest = activeMocks.get(0);
        if (autoAbandonStale && activeMocks.size() > 1) {
            LocalDateTime now = LocalDateTime.now();
            List<MockSession> stale = new ArrayList<>();
            for (int i = 1; i < activeMocks.size(); i++) {
                MockSession old = activeMocks.get(i);
                old.setStatus(MockStatus.ABANDONED);
                old.setEndTime(now);
                if (old.getStartTime() != null) {
                    old.setDurationSeconds(
                        (int) ChronoUnit.SECONDS.between(old.getStartTime(), now));
                }
                stale.add(old);
            }
            mockSessionRepository.saveAll(stale);
        }
    ...
    private void initializeAnswerSheet(MockSession session, List<Question> questions) {
        List<MockAnswer> answerRows = new ArrayList<>();
        for (Question question : questions) {
            ...
            answer.setAttempted(false);
            answer.setMarkedReview(false);
            answer.setUserAnswer(null);
            answer.setTimeTakenSeconds(0);
            answer.setResult(AnswerResult.SKIPPED);
            answer.setMarksAwarded(0);
            answerRows.add(answer);
        }
        mockAnswerRepository.saveAll(answerRows);
    }
    ...
    private MockSessionResponse mapToSessionResponse(
        ...
        response.setTimeLimitMinutes(Math.max(1, (questions.size() * 60) / 60));
