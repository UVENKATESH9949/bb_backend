...
public class MockAnswerServiceImpl implements MockAnswerService {
    ...
    public MockAnswerResponse saveAnswer(Long sessionId, MockAnswerRequest request) {
        ...
        MockAnswer answer = mockAnswerRepository
            .findByMockSessionIdAndQuestionId(sessionId, request.getQuestionId())
            .orElseGet(() -> {
                MockAnswer fresh = new MockAnswer();
                fresh.setMockSession(session);
                fresh.setQuestion(question);
                fresh.setSubject(question.getSubject());
                fresh.setTopic(question.getTopic());
                fresh.setDifficultyLevel(question.getDifficultyLevel());
                return fresh;
            });
    ...
    private void evaluateSingleAnswer(MockAnswer answer, Question question) {
        ...
        boolean isCorrect = isAnswerCorrect(question, userAnswer, correctAnswer);
    ...
    private boolean isAnswerCorrect(Question question, String userAnswer, String correctAnswer) {
        String normalizedUser = userAnswer == null ? "" : userAnswer.trim();
        String normalizedCorrect = correctAnswer == null ? "" : correctAnswer.trim();

        if (normalizedUser.isEmpty() || normalizedCorrect.isEmpty()) {
            return false;
        }

        if (question.getQuestionType() == QuestionType.MULTI_CORRECT) {
            List<String> correctSet = normalizeSet(normalizedCorrect);
            List<String> userSet = normalizeSet(normalizedUser);
            return !correctSet.isEmpty() && correctSet.equals(userSet);
        }

        if (question.getQuestionType() == QuestionType.CODE_FILL
                || question.getQuestionType() == QuestionType.CODE_OUTPUT
                || question.getQuestionType() == QuestionType.SQL_OUTPUT) {
            return normalizeSet(normalizedCorrect).contains(
                normalizedUser.toLowerCase(Locale.ROOT));
        }

        return normalizedUser.equalsIgnoreCase(normalizedCorrect);
    }

    private List<String> normalizeSet(String raw) {
